...
def build_s1(wb):
    ...
    r = team_progress_block(
        ws, r, "연장진열대 (단위: 점)",
        headers=["구분", "완료점(상반기)", "하반기 목표점", "합계"],
        region=[79, 26, 105],
        rows={
            "수원영업1팀": [14, 3, 17], "수원영업2팀": [9, 7, 16], "수원영업3팀": [4, 1, 5],
            "수원영업4팀": [6, 3, 9], "수원영업5팀": [6, 1, 7], "수원영업6팀": [9, 0, 9],
            "수원영업7팀": [11, 3, 14], "수원영업8팀": [11, 3, 14], "수원영업9팀": [9, 5, 14],
        },
    )
    r = bullets(ws, r, 7, [
        "  - 연장진열대 상반기 79점 완료, 하반기 26점 진행",
    ...
    r = team_progress_block(
        ...
        region=[108, 45, 153],
        rows={
            "수원영업1팀": [12, 2, 14], "수원영업2팀": [20, 3, 23], "수원영업3팀": [6, 2, 8],
            "수원영업4팀": [13, 8, 21], "수원영업5팀": [14, 12, 26], "수원영업6팀": [9, 11, 20],
            "수원영업7팀": [16, 2, 18], "수원영업8팀": [6, 4, 10], "수원영업9팀": [12, 1, 13],
        },
    )
    r = bullets(ws, r, 7, [
...
def build_check(wb):
    ...
    guide = ("· 부장님 피드백 : 팀별 하반기 목표는 좋으나 '7월에 몇 개 했는지' 체크 + 항목별 7월 전개 내용 기입 필요\n"
             "· 노란색 칸(7월 완료 / 7월 전개 내용)만 입력하면, 7월 누계·달성률은 자동 계산됩니다.  "
             "달성률 = 7월 완료 ÷ 하반기 목표")
    put(ws, "A2", guide, Font(name="맑은 고딕", size=9, color="595959"), LEFT, GREY, border=False)

    headers = ["전략 / 항목", "팀", "상반기 완료", "하반기 목표",
               "7월 완료(입력)", "7월 누계", "달성률", "7월 전개 내용(입력)"]
    hr = 5
    for i, h in enumerate(headers):
        put(ws, f"{get_column_letter(1 + i)}{hr}", h, HEAD_FONT, CENTER, BLUE)
    ws.row_dimensions[hr].height = 22

    # 각 항목: (이름, {팀:(상반기완료, 하반기목표)}, 지역부(상반기완료,하반기목표))
    # 진행점만 있는 항목은 상반기완료=None, 하반기목표=진행점
    items = [
        ("① 연장진열대",
         {"수원영업1팀": (14, 3), "수원영업2팀": (9, 7), "수원영업3팀": (4, 1), "수원영업4팀": (6, 3),
          "수원영업5팀": (6, 1), "수원영업6팀": (9, 0), "수원영업7팀": (11, 3), "수원영업8팀": (11, 3),
          "수원영업9팀": (9, 5)}, (79, 26)),
        ("② 점두진열",
         {"수원영업1팀": (12, 2), "수원영업2팀": (20, 3), "수원영업3팀": (6, 2), "수원영업4팀": (13, 8),
          "수원영업5팀": (14, 12), "수원영업6팀": (9, 11), "수원영업7팀": (16, 2), "수원영업8팀": (6, 4),
          "수원영업9팀": (12, 1)}, (108, 45)),
        ("③ 주먹밥+음료(진행점)",
         {"수원영업1팀": (None, 14), "수원영업2팀": (None, 13), "수원영업3팀": (None, 9), "수원영업4팀": (None, 10),
          "수원영업5팀": (None, 12), "수원영업6팀": (None, 13), "수원영업7팀": (None, 6), "수원영업8팀": (None, 1),
          "수원영업9팀": (None, 9)}, (None, 87)),
        ("④ 헬시플레저 샌드위치(진행점)",
         {"수원영업1팀": (None, 14), "수원영업2팀": (None, 18), "수원영업3팀": (None, 6), "수원영업4팀": (None, 10),
          "수원영업5팀": (None, 12), "수원영업6팀": (None, 10), "수원영업7팀": (None, 6), "수원영업8팀": (None, 9),
          "수원영업9팀": (None, 12)}, (None, 97)),
        ("⑤ 간편식사 아크릴 진열대(진행점)",
         {"수원영업1팀": (None, 18), "수원영업2팀": (None, 22), "수원영업3팀": (None, 25), "수원영업4팀": (None, 14),
          "수원영업5팀": (None, 13), "수원영업6팀": (None, 13), "수원영업7팀": (None, 13), "수원영업8팀": (None, 16),
          "수원영업9팀": (None, 13)}, (None, 147)),
        ("⑥ 라심발리 교체",
         {"수원영업1팀": (5, 1), "수원영업2팀": (8, 1), "수원영업3팀": (6, 4), "수원영업4팀": (5, 0),
          "수원영업5팀": (2, 0), "수원영업6팀": (5, 1), "수원영업7팀": (10, 2), "수원영업8팀": (10, 2),
          "수원영업9팀": (8, 1)}, (59, 12)),
    ]
    ...
    for name, teams, region in items:
        ...
        # 지역부 7월완료 = 팀 합계 수식, 누계/달성률도 수식
        team_rows = range(r + 1, r + 1 + len(teams))
        ...
        put(ws, f"H{r}", "", BOLD, LEFT, LIGHT)
        r += 1
        for team, (done, tgt) in teams.items():
            ...
            # 7월 완료 입력란 (노랑)
            put(ws, f"E{r}", None, BODY, CENTER, YELLOW)
            # 7월 누계 = 상반기완료 + 7월완료 (상반기 없으면 7월완료만)
            if done is not None:
            ...
            # 7월 전개 내용 입력란 (노랑)
            put(ws, f"H{r}", None, BODY, LEFT, YELLOW)
            r += 1
        # 항목 사이 간격
        r += 1
